...
public class World {
	...
	public void drawPath() {
		...
		StdDraw.picture(k.x, k.y,"images/path.png", squareWidth, squareHeight );
	...
	public void waves() {
		if(monsters.size()==0&&!end) {
			wave++;
			switch (wave) {
			case 1 : // Première vague
				for (int i=0; i<2; i++) {
					addBaseMonster();
				}
				break;
			case 2 : // Deuxième vague
				for (int i=0; i<5; i++) {
					addBaseMonster();
				}
				break;
			case 3 : // Troisième vague
				for (int i=0; i<5; i++) {
					addAirMonster();
				}
				break;
			case 4 : // Quatrième vague
				for (int i=0; i<1; i++) {
					addBossMonster();
				}
				break;
	...
	public void updateMonsters() {
		...
		do {
			...
			if(m.p.x>path.get(10).x&&m.virage>0) {
				m.nextP=path.get(17);
				m.virage=2;
			}
			if(m.p.y>(path.get(17).y-0.01)&&m.virage>1) {
				m.nextP=path.get(19);
				m.virage=3;
			}
			if(m.p.x>path.get(19).x) {
				m.nextP=path.get(24);
				m.virage=4;
			}
			if(m.p.y<(path.get(24).y+0.01)&&m.virage>3) {
				m.nextP=path.get(26);
				m.virage=5;
			}
			if(m.p.x>path.get(26).x) {
				m.nextP=path.get(32);
				m.virage=6;
			}
			if(m.p.y>path.get(32).y) {
				m.nextP=path.get(34);
				m.virage=7;
			}
			if(m.p.x>path.get(34).x) {
				m.nextP=path.get(43);
			}
