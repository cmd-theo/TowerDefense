...
public class World2 {
	...
	public World2(int width, int height, int nbSquareX, int nbSquareY, int startSquareX, int startSquareY) {
		this.width = width;
		this.height = height;
	...
	public void drawBackground() {	
		...
		for (int i = 0; i < nbSquareX; i++)
			for (int j = 0; j < nbSquareY; j++)
				StdDraw.picture(i * squareWidth + squareWidth / 2, j * squareHeight + squareHeight / 2, "images/world_n.PNG", squareWidth, squareHeight);
	}
	...
	public void drawPath() {
		...
		StdDraw.picture(k.x, k.y,"images/path_n.PNG", squareWidth, squareHeight );
		for(int i=0; i<path.size(); i++) {
			Position p = new Position(path.get(i));
			out.add(p);
			StdDraw.picture(p.x, p.y,"images/path_n.PNG", squareWidth, squareHeight );
		}
	}
	...
	public void updateMonsters() {
		...
		do {
			...
			if(m.p.y<path.get(5).y) {
				m.nextP = path.get(8);
				m.virage=1;
			}
			if(m.p.x>path.get(8).x&&m.virage>0) {
				m.nextP=path.get(14);
				m.virage=2;
			}
			if(m.p.y>(path.get(14).y-0.01)&&m.virage>1) {
				m.nextP=path.get(16);
				m.virage=3;
			}
			if(m.p.x>path.get(16).x) {
				m.nextP=path.get(26);
				m.virage=4;
			}
			if(m.p.y<(path.get(26).y+0.01)) {
				m.nextP=path.get(28);
				m.virage=5;
			}
			if(m.p.x>path.get(28).x) {
				m.nextP=path.get(38);
				m.virage=6;
			}
			if(m.p.y>path.get(38).y-0.01&&m.virage>5) {
				m.nextP=path.get(40);
				m.virage=7;
			}
			if(m.p.x>path.get(40).x) {
				m.nextP=path.get(49);
			}
